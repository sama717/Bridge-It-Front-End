...
//         <button  style={{fontSize:"12px",boxShadow:"0 4px 10px #00000026, 0 6px 15px #FFFFFF40"}} className="w-auto formbtn">Invite</button>
//         </div>
//         </div>
...
const Invitation = ({ groupId }) => {
  ...
  useEffect(() => {
    ...
    if (groupId || storedGroupId) {
      setInviteLink(`http://localhost:3000/invitation`);
    }
  }, [groupId]);
  ...
  return (
    <div className="forrm">
      ...
      <div className="text-dark fw-bold text-center mt-2 title" style={{ wordSpacing: "6px" }}>
        Your workspace has been created successfully!
      </div>
      <div className="mt-3 p2">Thank you for choosing us and our services.</div>

      <div className="invitee">
        <label style={{ color: "#6B7280", fontSize: "13px" }}>Invite your team members</label>
        <div className="d-flex align-items-center gap-2 mt-3">
          <input type="email" className="form-control" placeholder="Enter your team members’ email" />
          <button className="w-auto formbtn" style={{ fontSize: "12px", boxShadow: "0 4px 10px #00000026, 0 6px 15px #FFFFFF40" }}>
            Invite
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
        ...
        {!copied ? (
          <FontAwesomeIcon icon={faClone} className="ms-2" style={{ cursor: "pointer", color: "#004EA0" }} onClick={handleCopy} />
        ) : (
      ...
      <div style={{ color: "#4B5563", textAlign: "center", fontSize: "12px", marginTop: "10px" }}>
        If you face any problem, please contact us by <u>email</u>.
      </div>
    </div>
  );
};
